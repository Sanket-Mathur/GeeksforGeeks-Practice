...
public:
    ...
    {
        int index = 0;
        Node* root = helper(pre, size, INT_MIN, INT_MAX, index);
        return root;
    }
    
    Node* helper(int pre[], int size, int min, int max, int &ind) {
        if (ind >= size) {
            return NULL;
        }
        ...
        if (pre[ind] > min && pre[ind] < max) {
            ...
            root->left = helper(pre, size, min, tmp, ind);
            root->right = helper(pre, size, tmp, max, ind);
        }
